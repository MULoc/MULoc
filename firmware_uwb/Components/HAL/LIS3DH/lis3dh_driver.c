...
* Description    : LIS3DH_Init
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
u8_t LIS3DH_Init(void)
{
	u8_t LIS3DH_ID=0;          //定义设备ID
  u8_t response=0;
//	u8 n=0;

	response = LIS3DH_SetODR(LIS3DH_ODR_100Hz);
//	_dbg_printf("Set ODR:%d\r\n",response); 
	response = LIS3DH_SetMode(LIS3DH_NORMAL);
//	_dbg_printf("Set MODE:%d\r\n",response); 
	response = LIS3DH_SetFullScale(LIS3DH_FULLSCALE_2);
//	_dbg_printf("Set SCALE:%d\r\n",response); 
	response = LIS3DH_SetAxis(LIS3DH_X_ENABLE | LIS3DH_Y_ENABLE | LIS3DH_Z_ENABLE);
//	_dbg_printf("Set ENABLE:%d\r\n",response); 

	LIS3DH_GetWHO_AM_I(&LIS3DH_ID); 
	return LIS3DH_ID;
//  _dbg_printf("Device ID:0x%x\r\n",LIS3DH_ID); 
//	
//	n = sprintf((char*)&SendBuff[0],"Device ID:0x%x\r\n",LIS3DH_ID);
