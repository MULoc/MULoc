...
/* add diagnose */
rdio_rx_diag_t tag_final_diag;
void get_rx_diag_val(rdio_rx_diag_t *rdio_rx_diag)
{
	...
	dwt_readdignostics(&rx_diag);

	rdio_rx_diag->F_Path_Power_Level = 10 * log10((pow(rx_diag.firstPathAmp1,2) + pow(rx_diag.firstPathAmp2,2) + pow(rx_diag.firstPathAmp3,2))/pow(rx_diag.rxPreamCount,2)) - 113.77;
	rdio_rx_diag->RX_Level = 10 * log10(rx_diag.maxGrowthCIR * pow(2,17)/pow(rx_diag.rxPreamCount,2)) - 113.77;	
}
			...
			if(instance_data[instance].mode != LISTENER)
			{
				...
				switch(dw_event.msgu.frame[fcode_index])
				{
					...
					case RTLS_DEMO_MSG_ANCH_FINAL:
						...
						{
							if(dw_event.msgu.frame[fcode_index] == RTLS_DEMO_MSG_TAG_FINAL)
							{
								get_rx_diag_val(&tag_final_diag);
							}
						}
    ...
    if((instance_data[instance].instanceTimerEn == 1) && (instance_data[instance].stopTimer == 0))
    {
        if(instance_data[instance].mode == TAG)
        {
					if((portGetTickCount() - instance_data[instance].instanceWakeTime) > instance_data[instance].nextSleepPeriod)
					{
						...
						//_dbg_printf("PC timeout DWT_SIG_RX_TIMEOUT\n");
						instance_putevent(dw_event, DWT_SIG_RX_TIMEOUT);
					}
        }
