...
void  port_wakeup_dw1000(void)
{
		...
		dwt_seteui(instance_data[0].eui64);
}
...
int testapprun(instance_data_t *inst, int message)
{
    ...
    switch (inst->testAppState)
    {
		   ...
		   //_dbg_printf("TA_TX_WAIT_CONF %d m%d %d states %08x %08x\n", inst->previousState, message, inst->newReportSent, dwt_read32bitreg(0x19), dwt_read32bitreg(0x0f)) ;
				...
				if(dw_event->type != DWT_SIG_TX_DONE) //wait for TX done confirmation
                {
					if(dw_event->type != 0)
					{
						...
						{
							//_dbg_printf("RX timeout in TA_TX_WAIT_CONF (%d)\n", inst->previousState);
                ...
                {
					//_dbg_printf("got TO in TA_TX_WAIT_CONF\n");
        ...
        case TA_RX_WAIT_DATA :                                                                     // Wait RX data
		   //_dbg_printf("TA_RX_WAIT_DATA %d", message) ;
